...
function buildTasmotaTemplate(consumer) {
  ...
  const staleAfterS = consumer.stale_after_s || (consumer.poll_interval_s || 30) * 2;
...
.ec-inner { background:linear-gradient(160deg,#1e293b 0%,#0f172a 100%); border-radius:12px; padding:clamp(8px,2vw,14px); border:1px solid ${accent}88; box-shadow:0 4px 16px rgba(0,0,0,0.35); width:100%; }
.ec-head { display:flex; align-items:flex-start; justify-content:space-between; gap:8px; margin-bottom:10px; flex-wrap:wrap; }
...
.ec-section-label { font-size:0.58em; color:#94a3b8; text-transform:uppercase; letter-spacing:0.06em; margin-bottom:4px; }
.ec-metrics { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:clamp(4px,1.5vw,8px); }
...
.ec-value { font-size:clamp(0.78em,2vw,0.98em); font-weight:700; color:#f8fafc; margin-top:2px; line-height:1.1; }
.ec-relay { margin-top:8px; font-size:0.72em; color:#cbd5e1; }
.ec-meta { margin-top:8px; font-size:0.62em; color:#64748b; line-height:1.45; word-break:break-word; }
.ec-meta strong { color:#94a3b8; font-weight:600; }
.ec-actions { display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:10px; }
  ...
  <div class="ec-inner">
    <div class="ec-head">
      <div style="min-width:0;flex:1;">
        ...
        <div class="ec-id">${id} · Tasmota</div>
      </div>
    ...
    <div class="ec-section">
      <div class="ec-section-label">Live</div>
      <div class="ec-metrics">
        ...
        <div class="ec-metric"><div class="ec-label">PF</div><div class="ec-value">{{ formatPf(powerFactor) }}</div></div>
      </div>
    </div>
    <div class="ec-section">
      <div class="ec-section-label">Energy</div>
      <div class="ec-metrics">
        <div class="ec-metric"><div class="ec-label">Total kWh</div><div class="ec-value">{{ formatKwh(energyKwh) }}</div></div>
        <div class="ec-metric"><div class="ec-label">Today</div><div class="ec-value">{{ formatKwh(energyToday) }}</div></div>
        <div class="ec-metric"><div class="ec-label">Yesterday</div><div class="ec-value">{{ formatKwh(energyYesterday) }}</div></div>
        <div class="ec-metric"><div class="ec-label">Period Wh</div><div class="ec-value">{{ formatWh(energyPeriod) }}</div></div>
      </div>
    </div>
    <div class="ec-section">
      <div class="ec-section-label">Power quality</div>
      <div class="ec-metrics">
        <div class="ec-metric"><div class="ec-label">Apparent W</div><div class="ec-value">{{ formatW(apparentPower) }}</div></div>
        <div class="ec-metric"><div class="ec-label">Reactive W</div><div class="ec-value">{{ formatW(reactivePower) }}</div></div>
        <div class="ec-metric"><div class="ec-label">Relay</div><div class="ec-value" :style="{ color: switchOn ? '#34d399' : '#94a3b8' }">{{ relayLabel }}</div></div>
        <div class="ec-metric"><div class="ec-label">Source</div><div class="ec-value" style="font-size:0.65em;">{{ phaseSource }}</div></div>
      </div>
    </div>
    <div class="ec-meta">
      <div v-if="tasmotaTime"><strong>Device time</strong> {{ tasmotaTime }}</div>
      <div v-if="totalStartTime"><strong>Meter since</strong> {{ totalStartTime }}</div>
      <div v-if="tasmotaTopic"><strong>MQTT topic</strong> {{ tasmotaTopic }}</div>
    </div>
    ${switchRow}
  </div>
</div>
...
export default {
  ...
  methods: {
    ...
    formatKwh(v) { if (v == null || v === '') return '—'; const n = Number(v); return Number.isNaN(n) ? '—' : n.toFixed(3); },
    ...
    formatPf(v) { if (v == null || v === '') return '—'; const n = Number(v); return Number.isNaN(n) ? '—' : n.toFixed(2); },
    formatWh(v) { if (v == null || v === '') return '—'; const n = Number(v); return Number.isNaN(n) ? '—' : Math.round(n); },
    parseMs(v) {
  ...
  computed: {
    ...
    energyToday() { return this.ex.energy_today; },
    energyYesterday() { return this.ex.energy_yesterday; },
    energyPeriod() { return this.ex.energy_period; },
    apparentPower() { return this.ex.energy_apparentpower; },
    reactivePower() { return this.ex.energy_reactivepower; },
    powerFactor() { return this.ex.energy_factor; },
    totalStartTime() { return this.ex.energy_totalstarttime || ''; },
    tasmotaTime() { return this.ex.tasmota_time || ''; },
    tasmotaTopic() { return this.ex.tasmota_topic || ''; },
    phaseSource() { return this.ex.phase_source || this.p.source || '—'; },
    switchOn() {
      if (this.ex.switch_on !== undefined) return !!this.ex.switch_on;
      return null;
    },
    relayLabel() {
      if (this.switchOn === null) return '—';
      return this.switchOn ? 'ON' : 'OFF';
    },
...
function buildTemplate(consumer) {
  if (consumer.type === "tasmota_meter") {
    return buildTasmotaTemplate(consumer);
  }
  const id = consumer.id;
...
for (const consumer of consumers) {
  ...
  nodes.push({
    ...
    page: "ui_page_energy",
    width: "12",
    height: consumer.type === "tasmota_meter" ? "3" : "1",
    order,
    showTitle: false,
    className: "",
    visible: "true",
